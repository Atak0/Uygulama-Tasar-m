...
==========================================================
DENEY AŞAMASI 4 — CLAHE Varyasyonu (düzeltilmiş)
...
def preprocess_image(image_path, apply_clahe_flag=True):
    """
    Görüntüyü yükle, CLAHE uygula ve [0, 255] ölçeğinde döndür.

    Neden [0, 255]?
    ----------------------------
    Bu TensorFlow/Keras EfficientNetB0 sürümünde modelin başında zaten
    Rescaling(1/255) katmanı var. Burada tekrar [0,1]'e bölmek girişi
    ikinci kez küçültür.
    """
...
def load_dataset(dataset_dir=DATASET_DIR):
    ...
    print(f"Piksel   : [{images.min():.3f}, {images.max():.3f}]  -> [0,255] olmali")
...
if __name__ == "__main__":
    print("=" * 55)
    print("MODÜL 1 (v2) — Elastic + Grid + CLAHE Varyasyonu")
    print("=" * 55)
    ...
    print("\n[✓] Pipeline hazır!")
    print("    Aktif: Elastic + Grid + CLAHE(rand) + RandomBrightnessContrast")

"""
Spinal-AI2024 Skolyoz Test Pipeline - ResNet50V2
================================================
Bu script:
...
OUTPUT_DIR = BASE_DIR / "spinal_test_results_resnet50v2"
...
def list_saved_resnet_models():
    ...
    return sorted(set(model_paths), key=resnet_version_score, reverse=True)


def resolve_model_path():
    try:
        return resolve_model_path_common(WEIGHTS_PATH, family="resnet50v2", default_path=WEIGHTS_PATH)
...
def build_resnet50v2_model(model_path=None):
    import tensorflow as tf
    from tensorflow.keras import layers, Model
    from tensorflow.keras.applications import ResNet50V2

    variant = infer_variant(model_path) if model_path is not None else "v4"
...
def load_model():
    ...
    print(f"[↑] ResNet50V2 modeli yukleniyor: {model_path}")
    try:
        ...
        model = build_resnet50v2_model(model_path)
...
def print_report(report):
    print("\n" + "=" * 55)
    print("  SKOLYOZ TESPIT RAPORU — ResNet50V2")
    print("=" * 55)
...
def save_outputs(results, report):
    ...
    fig.suptitle("Spinal-AI2024 × ResNet50V2 — Skolyoz Tespit Raporu",
...
def main():
    print("=" * 55)
    print("  Spinal-AI2024 Skolyoz Test Pipeline - ResNet50V2")
    print("=" * 55)

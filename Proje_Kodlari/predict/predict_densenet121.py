"""
DenseNet121 X-ray tahmin araci.

Kullanim:
    python model_egitim/predict/DenseNet121/predict_densenet121.py --image "C:\\path\\xray.jpg"
    python model_egitim/predict/DenseNet121/predict_densenet121.py --folder "C:\\path\\external_images"
"""
...
DEFAULT_MODEL_DIR = BASE_DIR / "Models" / "Model_DenseNet121v1"
WEIGHTS_PATH = str(DEFAULT_MODEL_DIR / "model_densenet121.h5")
...
def build_densenet121_model(num_classes=3, dropout_rate=0.5):
    ...
    return Model(inputs=inputs, outputs=outputs, name="spine_densenet121")


def list_saved_densenet_models():
    return [
        path for path in list_model_files(BASE_DIR / "Models")
        if "DenseNet121" in str(path) or "densenet" in path.name.lower()
    ]


def find_default_model_path():
    try:
        return resolve_model_path(WEIGHTS_PATH, family="densenet121")
    except FileNotFoundError:
        latest = latest_model_file("densenet121", prefix="Model_DenseNet121v")
        return latest if latest else DEFAULT_MODEL_DIR


def load_trained_model(model_path):
    model_path = resolve_model_path(model_path, family="densenet121", default_path=WEIGHTS_PATH)

    try:
        return tf.keras.models.load_model(model_path, compile=False), "tam model"
    except Exception as load_model_error:
        model = build_densenet121_model(num_classes=len(CLASS_NAMES))
        try:
            model.load_weights(model_path)
            return model, "agirlik dosyasi"
        except Exception as load_weights_error:
            raise RuntimeError(
                "DenseNet121 modeli yuklenemedi. Dosya tam model degilse "
                "DenseNet121 egitim mimarisiyle uyumlu agirlik dosyasi olmali.\n"
...
def parse_args():
    parser = argparse.ArgumentParser(description="DenseNet121 ile X-ray tahmini")
    parser.add_argument("--image", help="Tek goruntu yolu.")
    parser.add_argument("--folder", help="Bir klasordeki tum goruntuleri tahmin et.")
    parser.add_argument("--model", default=str(find_default_model_path()), help="Kullanilacak DenseNet121 model/agÄ±rlik dosyasi.")
